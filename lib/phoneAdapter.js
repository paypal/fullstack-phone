 ...
 */
function validatePhoneNumber(canonicalPhone, regionCode) {
    regionCode = legacyRegionCodeMap[regionCode] || regionCode;
    ...
    if (regionCode) {
        validFlag = phoneUtil.isValidNumberForRegion(phoneNumber, regionCode);
    ...
    // Some other error
    return new Error(errorCode); // TODO: this results in empty error messages, so FIX IT
}
